...
class Verify {
    ...
    public function entry($id = '') {
        ...
        if($this->useZh){ // 中文验证码
            ...
            for ($i = 0; $i<$this->length; $i++) {
                ...
                $codeNX  += mt_rand($this->fontSize*1.2, $this->fontSize*1.25);
                imagettftext($this->_image, $this->fontSize, mt_rand(-5, 5), $codeNX, $this->fontSize*1.8, $this->_color, $this->fontttf, $code[$i]);
            }
        }
        $code2       =   strtoupper(implode('', $code));
        session('verify_code',$code2);
    ...
    private function _writeLine(){
        ...
        //$imgof = $tximgw/2;
        //$img = imagecreate($this->imageW, $this->imageH);
        ...
        for($i=0; $i<$linect; ++$i){  
            ...
            $x1 = rand(0, $imgw);  
            $y1 = rand(0, $imgh);  
            imageline ($this->_image, $x, $y, $x1, $y1, imagecolorallocate($this->_image, rand(0,255), rand(0,255), rand(0,255)));  
        }
    }
